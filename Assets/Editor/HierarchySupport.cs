...
namespace Assets.Editor
{
    ...
    public class HierarchyHighlightManager
    {
        ...
        //[CustomEditor(typeof(HierarchyHighlightManager))]
        private static void SetObjectMenu(GameObject _obj, int _id_selection, Rect _rect_selection)
        {
            ...
            if (_selected && HierarchyNameTagMgr.GUI_SKIN != null)
            {
                bool _is_actived = _obj.activeSelf;
                Color _prev_col = GUI.color;
                if (_hover_toggle)
                    toggle_color = Color.Lerp(toggle_color, new Color(1f, 1f, 1f, 1f), 0.01f);
                else
                    toggle_color = Color.Lerp(toggle_color, new Color(1f, 1f, 1f, 0.5f), 0.01f);
