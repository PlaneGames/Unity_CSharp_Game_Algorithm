    ...
    public class IconDictionaryCreator : AssetPostprocessor
    {
        ...
        private static bool ContainsIconAsset(string[] assets)
        {
            foreach (string str in assets)
            {
                ...
                if (ReplaceSeparatorChar(Path.GetDirectoryName(str)) == "Assets/Toolkit/" + AssetsPath)
                {
                    return true;
                }
            }
        ...
        internal static void BuildDictionary()
        {
            ...
            var dir = new DirectoryInfo(Application.dataPath + "/Toolkit/" + AssetsPath);
            FileInfo[] info = dir.GetFiles("*.png");
            foreach(FileInfo f in info)
            {
                var texture = (Texture)AssetDatabase.LoadAssetAtPath($"Assets/Toolkit/SimpleFolderIcon/Icons/{f.Name}", typeof(Texture2D));
